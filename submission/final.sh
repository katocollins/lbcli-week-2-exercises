...
if [ "$FEE_SATS" -lt 1000 ] || [ "$FEE_SATS" -gt 5000 ]; then
  echo "Warning: Fee seems unusual."
else
  echo "Fee amount seems reasonable!"
fi

# =========================================================================
# CHALLENGE 4: Creating a Raw Transaction with RBF
# =========================================================================
echo ""
...
VERIFY_RBF_SEQ=$(echo "$DECODED_TX" | jq -r '.vin[0].sequence')
if [ "$VERIFY_RBF_SEQ" -lt 4294967294 ]; then
  VERIFY_RBF="true"
else
  VERIFY_RBF="false"
fi
...
# Use vout 1 (40648 sat — larger output)
